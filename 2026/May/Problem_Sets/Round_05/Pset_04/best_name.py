...
def best_name(filename : str) -> str:
    with open(filename, "r") as file:
        ...
        all_names = [name for name in names_info.keys()]
        defeated_names = []
        for values in names_info.values():
            for name in all_names:
                if name in values:
                    defeated_names.append(name)
        undefeated_names = []
        for name in all_names:
            if name not in defeated_names:
                undefeated_names.append(name)
        if not undefeated_names:
            return "INVALID"
        elif len(undefeated_names) > 1:
            return "UNKNOWN"
        return undefeated_names[0]
