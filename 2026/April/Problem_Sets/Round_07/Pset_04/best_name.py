...
def best_name(filename : str) -> str:
    with open(filename, "r") as file:
        ...
        valid_names = [name for name in names_dict.keys()]
        # reference to create list comprehension for double for loop: https://docs.python.org/3/reference/expressions.html#displays-for-lists-sets-and-dictionaries
        names_to_remove = [name for val in names_dict.values() for name in valid_names if name in val]
        best_names = [name for name in valid_names if name not in names_to_remove]
        if not best_names:
            return "INVALID"
