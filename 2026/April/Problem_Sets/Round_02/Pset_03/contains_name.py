...
def contains_name(filename : str, name : str) -> bool:
    with open(filename, "r") as file:
        ...
        if not content:
            return False
        # create list of lists based on file content
        ...
        # create variable to store and reference column names easier
        categories = updated_content[0]
        # remove redundant column names's list
        updated_content.pop(0)
        # check if 'name' column exists and note it's element's index if found
        name_index = None
        for col_name in range(len(categories)):
            if categories[col_name] == "name":
                name_index = col_name
        # if no 'name' column found, return False
        if name_index is None:
            return False
        # search for requested name and return boolean outcome
