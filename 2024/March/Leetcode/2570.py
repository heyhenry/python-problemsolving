...
def merge_arrays(nums1 : list[list[int]], nums2 : list[list[int]]) -> list[list[int]]:
    ...
    for row in range(len(nums1)):
        if nums1[row][0] not in nums:
            nums[nums1[row][0]] = nums1[row][1]
    
    for row in range(len(nums2)):
        if nums2[row][0] not in nums:
            nums[nums2[row][0]] = nums2[row][1]
        else:
            nums[nums2[row][0]] += nums2[row][1]
...
def main():
    ...
    print(merge_arrays(nums1 = [[2,4],[3,6],[5,5]], nums2 = [[1,3],[4,3]]))
